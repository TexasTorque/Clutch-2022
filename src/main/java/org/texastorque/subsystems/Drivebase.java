...
public final class Drivebase extends TorqueSubsystem implements Subsystems {
    ...
    private static final Translation2d locationBackLeft = new Translation2d(DISTANCE_TO_CENTER_X,
            -DISTANCE_TO_CENTER_Y),
    ...
    private Drivebase() {
        ...
        // odometry = new SwerveDriveOdometry(kinematics, gyro.getRotation2dClockwise().times(-1));
        ...
        // odometry.resetPosition(new Pose2d(.25, 2.2, gyro.getRotation2dClockwise().times(-1)),
        //         gyro.getRotation2dClockwise().times(-1));
        poseEstimator.resetPosition(new Pose2d(2.3, .36, gyro.getRotation2dClockwise().times(-1)),
                gyro.getRotation2dClockwise().times(-1));
    ...
    public final void update(final TorqueMode mode) {
        if (shooter.getCamera().hasTargets()) {
            updateWithAprilTag();
        }
    ...
    public final Pose2d getPose() {
        return getPoseEstimated();
    }

    public final Pose2d getPoseEstimated() {
    ...
    public final void log() {
        ...
        SmartDashboard.putString("FusedPos",
                String.format("(%02.3f, %02.3f)", getPoseEstimated().getX(), getPoseEstimated().getY()));
    ...
    //     try {
    //         Pose2d pose = TorqueLight.getRobotPose(getGyro().getRotation2dCounterClockwise(), Rotation2d.fromDegrees(shooter.getCamera().getTargetPitch()), Rotation2d.fromDegrees(shooter.getCamera().getTargetYaw()), Shooter.TARGET_HEIGHT,
    ...
    //         poseEstimator.addVisionMeasurement(pose, TorqueUtil.time() - shooter.getCamera().getLatency() / 1000. - .0011);
    ...
    private final void updateWithAprilTag() {
        Pose2d pose = shooter.getCamera().getRobotPoseAprilTag(aprilTags, Shooter.CAMERA_ANGLE.getDegrees(), Shooter.CAMERA_HEIGHT, gyro.getRotation2dCounterClockwise(), turret.getDegrees());
        SmartDashboard.putString("AprilPos", String.format("(%02.3f, %02.3f)", pose.getX(), pose.getY()));
        aprilField.setRobotPose(pose);
        poseEstimator.addVisionMeasurement(pose, TorqueUtil.time() - shooter.getCamera().getLatency() / 1000.);
    }
    // /**
